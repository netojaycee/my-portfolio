...
Personal portfolio site (Next.js, Prisma 7 + Neon serverless adapter, Tailwind v4). Built from an upfront spec — this file is now the single source of truth for that spec (tech stack, file structure, section-by-section UI, aesthetic direction, agent rules); it used to be split across a root `CLAUDE.md` and `docs/CLAUDE.md`, merged here so there's one file to read. `docs/ARCHITECTURE.md` has the data model, seed content, and static copy. `docs/current_status.md` tracks phase-by-phase build progress — the build is complete, so treat the spec below as the as-built reference, not a to-do list, and check `docs/current_status.md` / the actual code when in doubt about current state.

## CV / experience log

The engineering work on this project is tracked for CV-building purposes in `~/projects/me/my-experience.md`, under its `## My Portfolio` section (that file has one section per project across everything the user works on — see `~/projects/me/status.md` for the cross-project tracker this belongs to). Whenever a session produces a new learning point worth that file — a non-trivial architectural decision (and the reasoning), a hard bug diagnosed and fixed, a security or performance fix, or a piece of engineering process enforced — append a concrete entry to that section before ending the session. Create the section if it's missing. Write entries as resume-bullet-ready facts: specific problem → specific solution → specific impact, not generic summaries, and not anything already fully covered there. This is a small personal project — keep entries proportionate to actual substance rather than padding them. Don't touch other projects' sections in that file.

---

# Original build brief

This is the brief originally written for the Claude Code agent that built this portfolio (previously `docs/CLAUDE.md`). Kept verbatim as the reference for tech stack, file structure, section-by-section UI spec, and admin panel spec — useful context for any future work on the site, even though the initial build is done.
